    ...
    public class MonthlyEmailAppService
    {
        ...
        public async Task EjecutarEnvioMensualAsync(CancellationToken ct = default)
        {
            ...
            foreach (var u in pendientes)
            {
                ...
                try
                {
                    var nombre = u.Person?.firstName     ?? "Usuario";
                    var html = EmailTemplates.CodigoVerificacion(nombre, u.EmailVerificationCode);
                    await _email.EnviarHtmlAsync(u.email!, "Confirma tu correo (código)", html);
                    enviados++;
                }
                catch (Exception ex)
                {
