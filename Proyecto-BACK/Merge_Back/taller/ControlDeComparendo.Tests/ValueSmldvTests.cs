...
namespace ControlDeComparendo.Tests
{
    public class ValueSmldvTests
    {
        ...
        public void Can_Create_ValueSmldv_With_Properties()
        {
            ...
            {
                id = 1,
                value_smldv = 150000.50,
                Current_Year = 2025,
                minimunWage = 1300000
            };

            Assert.Equal(1, smldv.id);
            Assert.Equal(150000.50, smldv.value_smldv);
            Assert.Equal(2025, smldv.Current_Year);
            Assert.Equal(1300000, smldv.minimunWage);
            Assert.Null(smldv.fineCalculationDetail); // ICollection no inicializada
        }
    }
}
