// using Google.Apis.Util;  // ❌ quítalo
...
public class AuthSessionService : IAuthSessionService
{
    ...
    public AuthSessionService(IAuthSessionRepository repo, ISystemClock clock, IConfiguration cfg, ILogger<AuthSessionService> log) // ✅
    {
        _repo = repo;
        _clock = clock;
        _log = log;
    ...
    {
        var now = _clock.UtcNow.DateTime; // ✅ Convert to DateTime
        var s = new AuthSession
        {
            SessionId = new Random().Next(1, int.MaxValue), // Generate random int instead of Guid
            PersonId = personId,
            CreatedAt = now,
            LastActivityAt = now,
    ...
    public async Task<(bool ok, ClaimsPrincipal? user)> ValidateAsync(int sessionId)
    {
        var now = _clock.UtcNow.DateTime; // Convert to DateTime
        ...
        { _log.LogInformation("Session {sid} absolute expired at {abs} (now {now})", s.SessionId, s.AbsoluteExpiresAt, now); return (false, null); }

        var idle = now - s.LastActivityAt;
        if (idle > _idle)
        { _log.LogInformation("Session {sid} idle expired: idle {idle} > limit {_idle}", s.SessionId, idle, _idle); return (false, null); }
    ...
    public Task RevokeAsync(int sessionId) => _repo.RevokeAsync(sessionId);
}
